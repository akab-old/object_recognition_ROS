...
class Scene {
    public:
        ...
        Scene::Ptr createSceneObj(PCLPointCloud2ConstPtr source_cloud){
...
public:
	...
	Recognition(PCLPointCloud2ConstPtr c){
        Scene app;
    ...
    PCLPointCloud2 run(){
        ...
        // for(vector<Table::Ptr>::iterator table = detectedTables->begin(); table != detectedTables->end(); ++table){
        //     PointCloud<PointT> table_cloud;
        //     copyPointCloud(*((*table)->getConvexHull()),table_cloud);
        //     toPCLPointCloud2(table_cloud,result);   
        // }
        toPCLPointCloud2(*(scene->getFullPointCloud()),result);
